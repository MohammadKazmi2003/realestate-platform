...
export function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <Link href={`/project/${project.slug}`} className="block shadow-neumorphic-outset hover:shadow-[6px_6px_12px_var(--shadow-dark),-6px_-6px_12px_var(--shadow-light)] transition-all duration-300 rounded-3xl p-1 group flex flex-col bg-bg-color h-full">
      ...
      <div className="flex flex-col flex-grow p-4">
        ...
        <div className="mt-2">
          <p className="text-xl font-bold text-success-color flex items-center">
            <IndianRupee size={18} />
            {project.low_price ? `${(project.low_price / 100000).toFixed(0)}L - ${(project.high_price / 100000).toFixed(0)}L` : 'Price on request'}
          </p>
        </div>
