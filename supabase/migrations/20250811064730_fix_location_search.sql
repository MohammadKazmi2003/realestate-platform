...
-- search_all_properties function to fix the location search logic.
...
-- Step 2: Recreate the unified_listings_view with the corrected location logic for properties
CREATE OR REPLACE VIEW public.unified_listings_view AS
SELECT
    -- Projects section
    p.id, p.name AS title, p.description_html AS description, p.low_price AS price,
    '/project/' || p.slug AS page_link, 'project' AS listing_type,
...
SELECT
    -- Properties section (with corrected location logic)
    prop.id, prop.title, prop.description, prop.price,
    '/property/' || prop.id::text AS page_link, 'property' AS listing_type,
    ...
    -- FIX: Get location from the property_locations join table
