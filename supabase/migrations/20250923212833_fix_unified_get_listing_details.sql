    ...
    ELSIF listing_type_from_view = 'project' THEN
        SELECT
            jsonb_build_object(
                ...
                -- *** FIX: Use the correct 'low_price' and 'high_price' columns from the projects table ***
                ...
                'property_types', (SELECT COALESCE(jsonb_agg(pt.name), '[]'::jsonb) FROM unit_configurations uc JOIN property_types pt ON uc.property_type = pt.id WHERE uc.project_id = proj.id),
                ...
                -- *** FIX: Use the correct 'project_amenities' junction table ***
