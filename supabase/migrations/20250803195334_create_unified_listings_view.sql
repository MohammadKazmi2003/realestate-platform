-- This migration updates the unified_listings_view to include the new
-- description_embedding column, making it available for semantic search.

-- DROP the old view first to modify its columns
DROP VIEW IF EXISTS public.unified_listings_view;

-- CREATE the new, updated VIEW
...
SELECT
    ...
    '/project/' || p.slug AS page_link, -- Corrected link construction
    'project' AS listing_type,
    p.description_embedding, -- ADDED embedding column
...
SELECT
    ...
    prop.description_embedding, -- ADDED embedding column
