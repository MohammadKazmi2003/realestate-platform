-- This function fetches all details for a single property,
-- including all related data from junction tables, and formats it as JSON.
-- VERSION 2: Adds COALESCE to ensure empty arrays instead of null.
CREATE OR REPLACE FUNCTION get_property_details(p_property_id uuid)
RETURNS jsonb -- Return a single JSONB object that matches your PropertyDataType
LANGUAGE sql
STABLE
AS $$
SELECT
    jsonb_build_object(
        ...
        -- FIX: Wrap one-to-one details in COALESCE to handle cases where they don't exist
        'details_residential', COALESCE((
            SELECT jsonb_agg(dr_agg)
            FROM (
                SELECT dr.*, to_jsonb(bt) as bhk_types, to_jsonb(lfs) as lookup_furnishing_statuses
                FROM details_residential dr
                LEFT JOIN bhk_types bt ON dr.bhk_type_id = bt.id
                LEFT JOIN lookup_furnishing_statuses lfs ON dr.furnishing_status_id = lfs.id
                WHERE dr.property_id = p.id
            ) dr_agg
        ), '[]'::jsonb),
        'details_commercial', COALESCE((
             SELECT jsonb_agg(dc_agg)
            FROM (
                SELECT dc.*, to_jsonb(lcst) as lookup_commercial_sub_types
                FROM details_commercial dc
                LEFT JOIN lookup_commercial_sub_types lcst ON dc.commercial_sub_type_id = lcst.id
                WHERE dc.property_id = p.id
            ) dc_agg
        ), '[]'::jsonb),

        -- FIX: Ensure all many-to-many relationships return an empty array [] if no items are found
