    ...
    ELSIF listing_type_from_view = 'project' THEN
        SELECT
            jsonb_build_object(
                ...
                -- *** FIX: Replaced incorrect table name with the correct 'unit_configurations' table ***
                'property_types', (SELECT COALESCE(jsonb_agg(pt.*), '[]'::jsonb) FROM unit_configurations uc JOIN property_types pt ON uc.property_type_id = pt.id WHERE uc.project_id = proj.id),
