-- Enrich rental + sale listing cards with full parity details.
-- Ensures every card-feeding RPC returns area (sq ft), area_unit, BHK/bedrooms,
-- bathrooms, balconies, cabins/workstations, furnishing_status and listing_purpose
-- so rental cards show the same rich details as for-sale cards.
...
BEGIN
    ...
    SELECT
        ...
        COALESCE(dl.plot_area, dr.carpet_area, dc.carpet_area) AS area_sqft,
        ...
        dr.balconies,
        dc.cabins,
        COALESCE(dc.workstations, dc.max_seats) AS workstations,
        fs.name AS furnishing_status,
    ...
    LEFT JOIN lookup_furnishing_statuses fs ON dr.furnishing_status_id = fs.id
...
-- 3. My listings cards
DROP FUNCTION IF EXISTS public.get_user_listings_with_all_images(uuid);
...
-- 4. Favorites cards
DROP FUNCTION IF EXISTS public.get_user_favorites_with_all_images(uuid);
