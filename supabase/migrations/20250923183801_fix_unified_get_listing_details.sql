...
BEGIN
    -- *** FIX: Changed column name from "type" to "listing_type" to match the view definition ***
    ...
    -- If it's a 'project', use a simplified project fetching logic
    ELSIF listing_type_from_view = 'project' THEN
        SELECT
            jsonb_build_object(
                ...
                'status', to_jsonb(ps),
                'property_types', (SELECT COALESCE(jsonb_agg(pt.*), '[]'::jsonb) FROM junction_project_property_types jppt JOIN property_types pt ON jppt.property_type_id = pt.id WHERE jppt.project_id = proj.id),
                'project_media', (SELECT COALESCE(jsonb_agg(pm.* ORDER BY pm.display_order), '[]'::jsonb) FROM project_images pm WHERE pm.project_id = proj.id),
                'amenities', (SELECT COALESCE(jsonb_agg(a.*), '[]'::jsonb) FROM junction_project_amenities jpa JOIN lookup_amenities a ON jpa.amenity_id = a.id WHERE jpa.project_id = proj.id)
            )
        INTO listing_details
        FROM
            projects proj
        LEFT JOIN developers dev ON proj.developer_id = dev.id
        LEFT JOIN project_statuses ps ON proj.status_id = ps.id
        LEFT JOIN locations loc ON proj.location_id = loc.id
        WHERE
            proj.id = p_listing_id
        GROUP BY -- *** FIX: Added GROUP BY for project query to handle potential multiple rows from joins ***
            proj.id, dev.id, ps.id, loc.id;
    END IF;
