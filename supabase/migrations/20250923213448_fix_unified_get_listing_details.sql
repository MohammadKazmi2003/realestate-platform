    ...
    ELSIF listing_type_from_view = 'project' THEN
        SELECT
            jsonb_build_object(
                ...
                -- *** FIX: Cast amenity_id to integer to match the lookup table's primary key type ***
                'amenities', (SELECT COALESCE(jsonb_agg(a.*), '[]'::jsonb) FROM project_amenities pa JOIN lookup_amenities a ON pa.amenity_id::integer = a.id WHERE pa.project_id = proj.id)
            )
        INTO listing_details
        FROM
            projects proj
