    ...
    ELSIF listing_type_from_view = 'project' THEN
        SELECT
            jsonb_build_object(
                ...
                -- *** FIX: Corrected the JOIN column from 'uc.property_type_id' to 'uc.property_type' as hinted by the error message ***
                'property_types', (SELECT COALESCE(jsonb_agg(pt.*), '[]'::jsonb) FROM unit_configurations uc JOIN property_types pt ON uc.property_type = pt.id WHERE uc.project_id = proj.id),
