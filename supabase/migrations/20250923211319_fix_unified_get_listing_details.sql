    ...
    ELSIF listing_type_from_view = 'project' THEN
        SELECT
            jsonb_build_object(
                ...
                'price', proj.low_price, -- Using low_price as the representative price for projects
                'location_text', loc.name,
                'developer', to_jsonb(dev),
                -- *** FIX: Replaced the incorrect JOIN with direct column selection ***
                ...
                'amenities', (SELECT COALESCE(jsonb_agg(a.*), '[]'::jsonb) FROM project_amenities pa JOIN lookup_amenities a ON pa.amenity_id = a.id WHERE pa.project_id = proj.id)
            )
        INTO listing_details
        FROM
            projects proj
